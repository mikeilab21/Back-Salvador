...
async function createUserAndUpdateIdentity(username, password, country, currency, countryCode, phoneNumber, firstName, lastName, address, city, birthDate, documentType, documentNumber, passportNumber, email) {
  ...
  const { cleanedCountryCode, cleanedPhoneNumber } = cleanCountryCode(countryCode, phoneNumber);

  // Concatenar cleanedCountryCode con cleanedPhoneNumber
  const numberWhatsapp = cleanedCountryCode + cleanedPhoneNumber;
